...
freezes the symbol and type names needed by the v1 Client; F02-S03 completes the public QP records
and attribute mask. WR and WC record fields remain owned by F02-S04, so those records are still
incomplete types.
...
| QP state attributes | `ugdr_qp_attr`, `ugdr_qp_attr_mask` | Complete state/current-state/access record and mask. Mask bits 0, 1, and 3 align with libibverbs. |
...
`ugdr_qp_attr` fixes `qp_state`, `cur_qp_state`, and `qp_access_flags`.
