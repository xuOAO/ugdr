...
1. 读取仓库 `AGENTS.md`、`docs/status/current.json`、`docs/status/roadmap.json` 和当前步骤的已审阅 Markdown。
...
- 已位于同步后的 `master`：`state show/next` 会校验 roadmap 所绑定的已审阅 Markdown revision/hash，并读取由它自动派生的 `next_actions`。没有 action 时报告；多个 action 时要求用户选择；唯一 action 且用户说“继续”时视为明确选择。来源过期、缺少 route 或状态不一致时停止，不从聊天或正文临时猜测。
...
- 门禁满足后补充 progress Acceptance，并用 `tools/ugdr state transition --to completed --updated-by human --human-confirmed` 更新状态；该转换必须从受审阅 roadmap 自动派生 `next_actions`，不接受手工注入。
